...
class Client(object):
    ...
    * Use disconnect() to disconnect from the broker
    ...
    def loop_forever(self, timeout=1.0, max_packets=1, retry_first_connection=False):
        ...
        return rc
    def loop_with_time(self,numero_segundos=2, timeout=1.0, max_packets=1, retry_first_connection=False):
        """This function call loop() for you in an infinite blocking loop. It
        is useful for the case where you only want to run the MQTT client loop
        in your program.
        ...
        """
        run = True
        timeout_loop = time.time() + numero_segundos

        while ((run) and (time.time() < timeout)):	
            print("loop1\n")
            if self._thread_terminate is True :
                break
...
#        while run:
        timeout_loop = time.time() + numero_segundos
        while ((run) and (time.time() < timeout_loop)):	
            print("loop 2\n")
            rc = MQTT_ERR_SUCCESS
            timeout_loop_01 = time.time() + numero_segundos
            while (rc == MQTT_ERR_SUCCESS) and (time.time() < timeout_loop_01):
                print("loop 3\n")
                rc = self.loop(timeout, max_packets)
                # We don't need to worry about locking here, because we've
                # either called loop_forever() when in single threaded mode, or
